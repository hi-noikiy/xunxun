...
//                $item[$v['room_mode']] = $list;
    ...
    public function getRoomList($token,$room_type,$page=1,$signature){
        ...
        try{
            ...
            //获取轮播图数据
            /*$type = 1;  //轮播类型1首页 2房间
            $banner = D('banner')->getAppUrl($type);*/
            //房间分类
            $status = 2;    //此把推荐的去掉
...
//            $room_mode = D('RoomMode')->getListes();
            ...
            if($data['room_type'] == 1){
                //热门数据
                $pageInfo = array("page" => $page, "pageNum"=>10, "totalPage" => 1);
//                $list = RoomtotalsService::getInstance()->getRedList($limit=10);
                $list = LanguageroomService::getInstance()->getRedList($limit=10);
                if($list){
                    foreach($list as $key=>$value){
                        $list[$key]['visitor_number'] = $value['visitor_number']+$value['visitor_externnumber'];//房间热度值
                        $list[$key]['room_image'] = MemberService::getInstance()->getOneByIdField($value['user_id'],"avatar");//用户头像
                        if(empty($list[$key]['room_image'])){
                            $list[$key]['room_image'] = C('APP_URL').'/Public/Uploads/image/logo.png';
                        }else{
...
//            var_dump($banner_list);die();
     ...
     * 返回值说明:
     */
    public function create($token,$room_name,$room_type,$device,$version,$room_tag=null,$signature){
       ...
       /* if($_FILES["room_image"]["error"] != 0){
        ...
        if(!$info) {// 上传错误提示错误信息
            ...
            $_POST["room_image"] = '/Public'.$info["room_image"]["savepath"].$info["room_image"]["savename"];
        }*/
        $token = I("post.token");
        $signature = I("post.signature");
...
//            "room_image"=> $_POST["room_image"],
        ...
        try{
            ...
            if(!$id){
                ...
                RoomtotalsService::getInstance()->addData($dataes);
                //添加房间麦位数据
                for($i=1;$i<=9;$i++){
                    // var_dump($value);die;
...
//            var_dump($user_info['user_id']);die();
            if($myroom_list){
