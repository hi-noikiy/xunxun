...
//		self::$ThirdPay = new ThirdPayController();
	...
	public function pay($action, $uid, $rmb, $token,$type,$is_active)
	{
	  ...
	  //  var_dump(self::$list);die;
		//检查必要参数
	  try{
		if(!isset($action) || !isset($uid) || !isset($rmb) || !isset($token)){
			E('参数不全',2000);
		}else{
	...
	//		var_dump($orderNo);die;
		//	var_dump($orderNo);die;
            if($type==2){      //type 为2表示vip购买
                ...
                $orderNo = $this->createOrder($uid,$rmb,$coin=$vip_days,$platform,$type,$is_active);
            }else if($type==1){
                $where['uid']=$userid;
                $coin = self::$list[$rmb]?self::$list[$rmb]["diamond"]:0;
                //var_dump($coin);die;
                //$coin = self::$list[$rmb]?self::$list[$rmb]["diamond"]+self::$list["$rmb"]["present"]:0;
                // 创建订单.
                $orderNo = $this->createOrder($uid,$rmb,$coin,$platform,$type,$is_active);
                //		var_dump($orderNo);die;
            }
			if (!$orderNo) {
				E('创建订单失败',2003);
	 ...
	 * @return mixed
	 */
	public function applePay($token,$uid, $rmb,$deaild,$certificate,$product_id='')
	{
	    try{
	        // 检查支付凭证.
...
//           $updatestatus=$model->where(array("status"=>0,"orderno"=>$orderNo))->setField(array('dealid','status','content','rmb'),array($dealid,1,$content,$rmb));
	 ...
	 * @return string
	 */
	private function createOrder($uid, $rmb, $coin, $platform=0,$type,$is_active,$title='', $content = '', $dealid = 0)
	{
		//生成订单号
