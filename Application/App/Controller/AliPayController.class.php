...
class AliPayController extends BaseController
{
    ...
    public function initmymoney($token){
        try{
            ...
            $cash=D('member')->getByqopenid($where,'diamond,free_diamond,totalcoin,freecoin');
            //剩余钻石数量
        ...
        try{
            ...
            if($exchangediamond>$diamond){
               ...
               $totalcoin= D('member')->setInccoin($where,$coin,'totalcoin');
                $free_diamond=D('member')->setInccoin($where,$exchangediamond,'free_diamond');
                $cashs=D('member')->getByqopenid($where,'diamond,free_diamond');
                            //剩余钻石数量
                $diamonds=$cashs[0]['diamond']-$cashs[0]['free_diamond'];
