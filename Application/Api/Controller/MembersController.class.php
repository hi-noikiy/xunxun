      ...
      //更新性别接口
   	...
   	public function updatesex($username,$sex){
   	    try{
   	        $field='id as userid,username,sex,nickname,intro,status,avatar,birthday,roomnumber';
   	        //实例化数据
            $user_id = D('member')->where(array("username"=>$username))->getField("id");
...
//            $user_info['type'] = 1;
	      $is_attention = D("attention")->is_attention($uid,$data['user_id']);
            ...
            $usernames=array(
                "usernames"=>array($data['user_id'])        
            );
            $result = $Easemob->addUserForBlacklist($uid,$usernames);       //拉黑操作
...
//            $room_mode = D('RoomMode')->getListes();
            ...
            if($data['room_type'] == 1){
                ...
                $list = D('languageroom')->getRedListess($limit=10);
//                $list = LanguageroomService::getInstance()->getRedListess($limit=10);
                if($list){
...
//                $object  = $dir . '/' . $v['name'];//想要保存文件的名称
                ...
                try {
                    if($is_safes[0] !== "pass"){
                        ...
                        $base_names = "/www/wwwroot/mtestapi.57xun.com";
                        $url = '/Public'.$info[$k]["savepath"].$info[$k]["savename"];
