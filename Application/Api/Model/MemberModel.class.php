...
//        echo $this->_Sql();
     ...
     * 查询列表
     */
    public function getlistAllByWhere($where,$field){
    ...
    public function getUserByInid($where,$field)
   {
     $data = array();
     $res = $this -> where($where) -> Field($field) -> select();
     foreach ($res as $key => $value) {
        // $value['avatar'] = $value['avatar']?C('IMG_URL').'/'.$value['avatar']:''; 
        $data[$value['uid']] = $value;
     }
     return $data;
   }
...
//        echo $this->_Sql();die();
     ...
     *获取用户审请数据
     */
    public function getOneByprettyId($id){
        $where = [];
        $where['pretty_id'] = $id;
  ...
  // }
     ...
     * @return mixed
     */
    public function getNickname($field){
...
//        echo $this->_Sql();die();
     ...
     * @return mixed
     */
    public function getOneByPrettyidField($id,$getfield){
        $where['pretty_id'] = $id;
        $field = $getfield;
...
//        echo $this->_Sql();die();
     ...
     * @return mixed
     */
    public function getOneByUserField($username,$getfield){
        $where['username'] = $username;
        $field = $getfield;
...
//		echo $this->_Sql();die();
     ...
     *获取该用户的详细信息field
     */
    public function getOneByIdsField($ids,$field){
...
//        echo $this->_Sql();
    ...
    public function detail($id){
        ...
        $field = "id as user_id,nickname,sex,avatar,is_vip,pretty_id,pretty_avatar,pretty_avatar_svga";
        $res = $this->field($field)->where($where)->find();
        if (!empty($res)) {
            $res['pretty_avatar'] = $res['pretty_avatar']?C('IMG_URL').'/'.$res['pretty_avatar']:'';
            $res['pretty_avatar_svga'] = $res['pretty_avatar_svga']?C('IMG_URL').'/'.$res['pretty_avatar_svga']:'';
        }
        return $res;
//        echo $this->_Sql();die();
    }
     ...
     * @return array|false|mixed|\PDOStatement|string|Model
     */
    public function userList($ids){
        $where['id'] = array("in",$ids);
        $field = "id as userid,sex,nickname,avatar,intro,pretty_id,pretty_avatar,pretty_avatar_svga";
        $res = $this->field($field)->where($where)->select();
        if (!empty($res)) {
            foreach ($res as $key => $value) {
                $res[$key]['pretty_avatar'] = $value['pretty_avatar']?C('IMG_URL').'/'.$value['pretty_avatar']:'';
                $res[$key]['pretty_avatar_svga'] = $value['pretty_avatar_svga']?C('IMG_URL').'/'.$value['pretty_avatar_svga']:'';
            }
        }
        return $res;
//        echo $this->_Sql();die();
    ...
    public function SearchMember($where){
        if (ctype_digit($where)) {
            $where = array("pretty_id = ' " . $where . " ' or nickname = '" . $where . "'");
        } else {
            $where = array("nickname = '" . $where . "'");
        }
        $field = "id as user_id,nickname,avatar,sex,pretty_id,pretty_avatar,pretty_avatar_svga";
        $res =  $this->field($field)->where($where)->select();
        if (!empty($res)) {
            foreach ($res as $key => $value) {
                $res[$key]['pretty_avatar'] = $value['pretty_avatar']?C('IMG_URL').'/'.$value['pretty_avatar']:'';
...
//        echo $this->_Sql();
    ...
    {
        $sql = "SELECT id as user_id,`nickname`,`avatar`,`sex`,`lv_dengji`,`is_vip`,`pretty_id`,`pretty_avatar`,`pretty_avatar_svga` FROM `zb_member` WHERE ( `pretty_id` LIKE '%".$search."%' ) OR ( `nickname` LIKE '%".$search."%' ) ORDER BY REPLACE(pretty_id,'".$search."','')";
        $res =  $this->query($sql);
        if (!empty($res)) {
            foreach ($res as $key => $value) {
                $res[$key]['pretty_avatar'] = $value['pretty_avatar']?C('IMG_URL').'/'.$value['pretty_avatar']:'';
...
//         }
        $field = "id as user_id,nickname,avatar,sex,lv_dengji,is_vip,pretty_id,pretty_avatar,pretty_avatar_svga";
...
//        echo $this->_Sql();die();
    ...
    public function UserDetail($user_id){
        ...
        $field = "id as user_id,username,nickname,sex,intro,avatar,is_vip,status,role,roomnumber,totalcoin,diamond,birthday,city,attestation,mobile,login_status,login_time,pretty_id,pretty_avatar,pretty_avatar_svga,lv_dengji,register_time";
        $res = $this->field($field)->where($where)->find();
        if (!empty($res)) {
            $res['pretty_avatar'] = $res['pretty_avatar']?C('IMG_URL').'/'.$res['pretty_avatar']:'';
            $res['pretty_avatar_svga'] = $res['pretty_avatar_svga']?C('IMG_URL').'/'.$res['pretty_avatar_svga']:'';
...
//        echo $this->_Sql();die();
    ...
    /**MB明細model
     */
//     public function mbdetails($where,$field){
//         return $this->field($field)->alias('m')
//                     ->join('zb_coindetail as c on m.id=c.uid')
//                     ->join('zb_beandetail as b on m.id=b.uid')                   
//                     ->where($where)                    
//                     ->select();
...
//     }
    ...
    public function user_vipinfo($user_id){
        ...
        $field='id as user_id,username,sex,nickname,intro,long_day,vip_buytime,pretty_id,pretty_avatar,pretty_avatar_svga';
        $res =  $this->field($field)->where($where)->find();
        if (!empty($res)) {
            $res['pretty_avatar'] = $res['pretty_avatar']?C('IMG_URL').'/'.$res['pretty_avatar']:'';
            $res['pretty_avatar_svga'] = $res['pretty_avatar_svga']?C('IMG_URL').'/'.$res['pretty_avatar_svga']:'';
...
//        echo $this->_Sql();die();
     ...
     * @param $user_id
     */
    public function getFieldCoin($user_id){
        $where['id'] = $user_id;
        $field='totalcoin,freecoin';
//        $field='IFNULL(totalcoin,0) - IFNULL(freecoin,0) as totalcoins';
        return $this->field($field)->where($where)->find();
//        echo $this->_Sql();die();
    }
